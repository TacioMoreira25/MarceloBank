...
public interface ContaRepository extends JpaRepository<Conta, Integer>
{
    ...
    @Query("SELECT c FROM Conta c WHERE c.numeroConta = :numero AND c.saldo " +
            "> :saldoMinimo")
    Optional<Conta> findContaComSaldoMinimo(@Param("numero") Integer numero,
                                            @Param("saldoMinimo") BigDecimal saldoMinimo);

    @Query("SELECT a.nomeAgencia, COALESCE(SUM(c.saldo), 0) FROM Agencia a LEFT JOIN " +
            "a.contas c GROUP BY a.codigoAgencia, a.nomeAgencia")
    List<Object[]> findSaldoTotalPorAgencia();

    @Query("SELECT c FROM Conta c WHERE c.saldo < 0")
    List<Conta> findContasComSaldoNegativo();

    @Query("SELECT c FROM Conta c LEFT JOIN FETCH c.cartoes WHERE c.numeroConta " +
            "= :numero")
